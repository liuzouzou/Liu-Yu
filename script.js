...
async function ensureLocalServer() {
  ...
  for (let i = 0; i < 20; i += 1) {
    try {
      ...
      if (res.ok) {
        window.location.replace(`${LOCAL_SERVER}/`);
        return;
      }
    } catch {
      /* 本地服务尚未启动，继续等待 */
...
// 其余问题 → POST /api/chat（由 chat.js 代理调用智谱 GLM）
// 本地请先运行 npm run open 或 npm start，再打开 http://localhost:3000
  ...
  try {
    ...
    const hint =
      window.location.protocol === "file:"
        ? "无法连接 AI：请先在终端运行 npm start，浏览器会自动打开 http://localhost:3000"
        : "无法连接 /api/chat：请先运行 npm start，再打开 http://localhost:3000";
