...
// 网站相关：三条建议问题 + 同类关键词 → 本地固定回答（不调用 apikey.md）
// 其余与网站无关的问题 → POST /api/chat（由 server.js 按 apikey.md 调用智谱 GLM）
...
- 爱好旅行、摄影，喜欢观察真实用户场景
- 邮箱：icey66@sina.com，电话：17301302676

回答要求：简洁友好的中文；只基于公开信息；无关问题简短回应并引导回刘宇相关话题。`;

function isDirectHtmlOpen() {
  return window.location.protocol === "file:";
}

function hasDirectApiKey() {
  return Boolean(window.ZHIPU_CHAT_CONFIG?.apiKey);
}

function getChatApiUrl() {
  if (isDirectHtmlOpen()) {
    return `http://127.0.0.1:${CHAT_SERVER_PORT}/api/chat`;
  }
  return "/api/chat";
}

function getSetupHint() {
  return "请先运行 npm run setup 或双击「打开网站.command」完成配置，再提问。";
}

async function isProxyAvailable() {
  try {
    const res = await fetch(`http://127.0.0.1:${CHAT_SERVER_PORT}/health`);
    return res.ok;
  } catch {
    return false;
  }
...
async function fetchAiReplyDirect() {
  const apiKey = window.ZHIPU_CHAT_CONFIG?.apiKey;
  if (!apiKey) throw new Error(getSetupHint());
  ...
  try {
    ...
    throw new Error("大模型返回异常，请稍后重试");
  }

  if (!response.ok) {
    throw new Error(
      data?.error?.message || data?.message || `大模型请求失败（${response.status}）`
    );
  }

  const content = parseModelContent(data);
  if (!content) throw new Error("大模型未返回有效内容");
  return content;
}

async function fetchAiReplyViaProxy() {
  ...
  try {
    ...
    throw new Error("本地 AI 服务异常，请运行 npm start 或 npm run setup");
  }
...
async function fetchAiReply() {
  if (!isDirectHtmlOpen()) {
    try {
      return await fetchAiReplyViaProxy();
    } catch (err) {
      if (hasDirectApiKey()) return fetchAiReplyDirect();
      throw err;
    }
  }

  const preferDirect = hasDirectApiKey();
  const proxyUp = await isProxyAvailable();

  if (preferDirect && !proxyUp) {
    return fetchAiReplyDirect();
  }
